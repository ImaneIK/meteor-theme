<template>
  <div>
    <si-SearchPost 
        v-if="$settings.sections.blog.banner.active"
        :image="$settings.sections.blog.banner.background.image"
        :title="$settings.sections.blog.banner.title.text"
        :searchbar="$settings.sections.blog.banner.searchbar.active"
        :placeholder="$settings.sections.blog.banner.searchbar.placeholder"
    />

    <div v-if="$settings.sections.blog.articles.active" class="mx-8 lg:mx-24 my-24 overflow-hidden">
    ...
    <div class="flex flex-col gap-4 md:flex-row  md:justify-between  items-center">
        <div class="flex flex-col justify-items-center lg:justify-start">
              <h1 class="text-2xl font-normal text-gray-800 lg:text-3xl">
                {{$settings.sections.blog.articles.title}}
              </h1>
