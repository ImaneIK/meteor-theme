<template>
    <div >
      ...
      <div v-if="loading" class="flex justify-center items-center h-screen">
        <div class="animate-spin w-8 h-8 border-t-2 border-gray-900"></div>
      </div>
  
      <div v-if="!loading && post != null" class="flex justify-between mx-16">
        ...
        <button @click="expandContent" class="text-xs text-white bg-amber-600 p-2 rounded-md cursor-pointer my-4 mx-auto">Read More</button>
      </div>
        </div>
        ...
        <div class="w-1/4 py-8 divide-x">
          <!-- Related Posts Block -->
          <div class="mb-8 p-4 bg-amber-100 text-center rounded-md shadow-md">
            <h2 class="text-xl font-semibold mb-4">Related Posts</h2>
            <ul>
              <li>
                <NuxtLink to="" class=" hover:underline">title 1</NuxtLink>
              </li>
              <li>
                <NuxtLink to="" class=" hover:underline">title 2</NuxtLink>
              </li>
              <li>
                <NuxtLink to="" class=" hover:underline">title 3</NuxtLink>
              </li>
  
              <!-- <li v-for="article in relatedPosts " :key="post.id" class="mb-2">
                <NuxtLink :to="`/blog/${post.slug}`" class="text-blue-600 hover:underline">{{ post.title }}</NuxtLink>
              </li> -->
            </ul>
          </div>
        ...
        <!-- ############################################################################# -->
          ...
          <div class="mb-8 bg-neutral-200 p-8 text-center rounded-md shadow-md">
            ...
            <button class="bg-black text-white text-xs p-2 rounded-md mt-2">Show more</button>
          </div>
