<template>
  <div class="container">
    ...
    <div
      ...
      class="flex flex-wrap items-center p-2 mx-2 bg-white border rounded"
    >
      <div class="w-full md:w-1/2">
        <svg
          class="h-40 max-w-full text-gray-400 translate"
      ...
      <div class="w-full pl-4 border-l md:w-1/2">
        ...
        <p>
          {{ "Go to your store admin > products > collections >" }}
          <a
            href="//business.storeino.com/stores/collections"
            target="_blank"
            class="text-blue-600"
            >{{ "Add new" }}</a
          >
        ...
        <a
          href="https://palest.storeino.com"
          target="_blank"
          class="inline-block p-2 mt-1 text-blue-600 border rounded-md"
          >{{ "View theme demo" }}</a
        >
      </div>
    </div>
    -->
    <!--  -->
    <div v-for="item in $settings.sections.collections">
    <Nuxt-link :to="`collections/${item.slug}`" > {{ item.name }}</Nuxt-link>
