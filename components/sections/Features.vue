<template>
<section :style="`background: ${$settings.theme.colors.secondary}; color: ${$settings.theme.colors.primary}`" class=" my-8" >
  <div class="mx-4 md:mx-12 max-w-screen-xl py-8 sm:py-12 lg:px-8 lg:py-16">
    ...
    >
      ...
      >
        ...
        <a
          :style="`background: ${$settings.theme.colors.accent}`"
          href="/shop"
      ...
      <div  class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
        ...
        >
          <span :style="`fill: ${$settings.theme.colors.primary}`" class="inline-block rounded-lg ">
          ...
          <p class="text-xs font-light py-1">{{ itemCount[i] }} {{$settings.sections.productdescription.items.label}}</p>
        </div>
