<template>
  <div v-if="$settings.sections.home.locations.active">
    ...
    <div v-if="!loading"
      :style="$settings.sections.home.locations.background ? { 'background': $settings.sections.home.locations.background } : { 'background': 'gray' }">

      <div class="my-12 px-2 lg:px-24 py-4  flex flex-col lg:gap-16 lg:flex-row items-center text-white ">
        <div>
          <h1 class="text-2xl font-normal text-white capitalize lg:text-3xl text-center lg:text-left">
            {{ $settings.sections.home.locations.title }}
          </h1>
        ...
        <div class="w-8/12 flex flex-col mx-auto">
          <div id="sliderContainer" class=" overflow-hidden">
            <ul ref="slider" id="slider" class="flex h-full overflow-hidden items-center">

              <li v-if="items.length > 0" v-for="(item, index) in items" :key="index" class=" p-5 overflow-hidden">
                <nuxt-link :to="`/collections/${item.slug}`"
                  class="block relative rounded-lg transition-all duration-300 group overflow-hidden ">

                  <div  class=" rounded-md overflow-hidden h-full group-hover:h-3/4 transition-all duration-300 ">
                    <nuxt-img v-if="item.image" class=" image h-72 w-full object-cover rounded-md hover:transition-all duration-300"
                  ...
                  <div
                    ...
                    <h2 class="flex items-center">
                      <svg class="block items-center" xmlns="http://www.w3.org/2000/svg" height="1em"
                        viewBox="0 0 384 512">
                        <style>
                          svg {
                            fill: #ff8000
                    ...
                    <p class="text-xs font-light text-gray-500"> {{ itemCount[index] }}
                      <!-- {{$settings.sections.productdescription.items.label}} -->
                    </p>
                    <!-- {{ itemCount[index] }} -->
                  </div>
              ...
              <li v-if="items.length == 0" class="relative border h-full py-16 flex flex-col items-center gap-8 rounded-md overflow-hidden h-72 flex justify-center text-center items-center ">
                <p class="p-12">No locations found</p>
              </li>
              ...
              <li class="p-5">
                <nuxt-link :to="`/shop`"
                  ...
                  <svg class="block fill-gray-600 p-4 w-1/3 h-1/3 object-cover rounded-md"
                    xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                    <path
                      d="M16.1 260.2c-22.6 12.9-20.5 47.3 3.6 57.3L160 376V479.3c0 18.1 14.6 32.7 32.7 32.7c9.7 0 18.9-4.3 25.1-11.8l62-74.3 123.9 51.6c18.9 7.9 40.8-4.5 43.9-24.7l64-416c1.9-12.1-3.4-24.3-13.5-31.2s-23.3-7.5-34-1.4l-448 256zm52.1 25.5L409.7 90.6 190.1 336l1.2 1L68.2 285.7zM403.3 425.4L236.7 355.9 450.8 116.6 403.3 425.4z" />
                  </svg>
                  <h2 class="block text-gray-500 text-center">
          ...
          <div  v-if="items.length != 0"  class=" flex gap-8 mx-8 justify-center lg:justify-end">
            <button @click="prev"
