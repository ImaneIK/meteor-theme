<template>
  <div class=" fixed w-screen bg-white dark:bg-black z-10 " style="padding: 16px;"  :class="['navbar', { 'navbar-transparent': $route.path === '/' && !scrolled, 'navbar-white': $route.path !== '/' || scrolled }]">
  
    <div class="flex items-center justify-between">

      <!-- Header logo -->
      <div class=" font-bold text-amber-600">
        <!-- <font-awesome-icon :icon="['fas', 'meteor']" /> -->
      ...
      <div class="hidden md:block">
        <ul class=" capitalize flex space-x-8 text-xs justify-center">

          <li v-for="link in $settings.header.menu.items" >
          <NuxtLink :to="link.url"
            :class="{
              'text-amber-500 underline decoration-solid decoration-amber underline-offset-4 decoration-2': $route.path === link.url, 
              'text-gray-400 font-light': $route.path !== link.url, 
            }"
          >{{link.text}}</NuxtLink>
          </li>
          
          <font-awesome-icon class="text-amber" :icon="['fas', 'magnifying-glass'] "></font-awesome-icon>
          <font-awesome-icon class="text-amber"  :icon="['far', 'user']" />
          <!-- <li><a href="#" class="cta bg-amber-600 hover:bg-amber-500 px-3 py-2 rounded text-white font-semibold">Get started</a></li> -->
        </ul>
      </div>
